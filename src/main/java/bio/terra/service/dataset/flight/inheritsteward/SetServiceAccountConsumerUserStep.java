...
public record SetServiceAccountConsumerUserStep(
  ...
  private StepResult setAuth(FlightContext flightContext, boolean inheritSteward)
    ...
    for (var projectId : Objects.requireNonNull(projectIds)) {
      if (inheritSteward) {
        resourceService.grantPoliciesServiceUsageConsumer(projectId, List.of(custodianEmail));
      } else {
        resourceService.revokePoliciesServiceUsageConsumer(projectId, List.of(custodianEmail));
      }
    }
