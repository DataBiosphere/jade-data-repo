...
public final class FireStoreProject {
  ...
  private FireStoreProject(String projectId) {
    ...
    this.firestore = FirestoreOptions.newBuilder().setProjectId(projectId).build().getService();
  }
  ...
  public static FireStoreProject get(String projectId) {
    ...
    // the same time and corrupt the map.  computeIfAbsent will avoid the relatively high cost of
    // building then
    // tossing a firestore project if it already exists. It is a way to combine checking for the key
    // and only
