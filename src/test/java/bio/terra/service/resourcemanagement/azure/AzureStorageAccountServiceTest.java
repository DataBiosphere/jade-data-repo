...
public class AzureStorageAccountServiceTest {
    ...
    private static final AzureStorageAccountSkuType STORAGE_SKU_TYPE = AzureStorageAccountSkuType.STANDARD_LRS;
    ...
    public void tesGetOrCreateStorageAccountCase1() throws InterruptedException {
        ...
        when(resourceDao.getStorageAccount(STORAGE_ACCOUNT_NAME, APPLICATION_DEPLOYMENT_NAME)).thenReturn(
    ...
    public void tesGetOrCreateStorageAccountCase5() {
        ...
        assertThrows(
            ...
            "Storage account does not exist, metadata out of sync with cloud state: " + STORAGE_ACCOUNT_NAME);
    }
    ...
    public void tesGetOrCreateStorageAccountCase7() throws InterruptedException {
        ...
        when(resourceDao.getStorageAccount(STORAGE_ACCOUNT_NAME, APPLICATION_DEPLOYMENT_NAME)).thenReturn(
            new AzureStorageAccountResource()
                ...
                .applicationResource(applicationResource)
                .region(REGION)
                // Locked by the same flight (something went wrong and storage account needs to be created for real)
                .flightId(flight));
        mockStorageAccountNotFound();
    ...
    public void tesGetOrCreateStorageAccountCase8() throws InterruptedException {
        String flight = ShortUUID.get();
        when(resourceDao.getStorageAccount(STORAGE_ACCOUNT_NAME, APPLICATION_DEPLOYMENT_NAME)).thenReturn(null);
        mockStorageAccountNotFound();
        mockStorageAccountCreation();
        // Mock creating the metadata record
        when(resourceDao.createAndLockStorageAccount(STORAGE_ACCOUNT_NAME, applicationResource, REGION, flight))
            .thenReturn(new AzureStorageAccountResource()
                .name(STORAGE_ACCOUNT_NAME)
                .applicationResource(applicationResource)
                .region(REGION)
    ...
    private void mockStorageAccountNotFound() {
        when(storageAccounts.getByResourceGroup(MANAGED_GROUP_NAME, STORAGE_ACCOUNT_NAME))
            .thenThrow(new ManagementException(
    ...
    private void mockStorageAccountCreation() {
        StorageAccount.DefinitionStages.Blank withRegion = mock(StorageAccount.DefinitionStages.Blank.class);
        StorageAccount.DefinitionStages.WithGroup withGroup = mock(StorageAccount.DefinitionStages.WithGroup.class);
        StorageAccount.DefinitionStages.WithCreate withSku = mock(StorageAccount.DefinitionStages.WithCreate.class);
        StorageAccount.DefinitionStages.WithCreate withHns = mock(StorageAccount.DefinitionStages.WithCreate.class);
        StorageAccount.DefinitionStages.WithCreate withCreate = mock(StorageAccount.DefinitionStages.WithCreate.class);
