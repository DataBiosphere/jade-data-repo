...
@javax.annotation.Generated(value = "io.swagger.codegen.languages.SpringCodegen", date = "2019-02-04T16:39:18.037-05:00")
...
public interface RepositoryApi {
    ...
    default ResponseEntity<JobModel> retrieveJob(@ApiParam(value = "A study or dataset id",required=true) @PathVariable("id") String id) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
                    return new ResponseEntity<>(getObjectMapper().get().readValue("{  \"submitted\" : \"submitted\",  \"description\" : \"description\",  \"id\" : { },  \"completed\" : \"completed\",  \"status\" : \"running\"}", JobModel.class), HttpStatus.NOT_IMPLEMENTED);
                } catch (IOException e) {
                    log.error("Couldn't serialize response for content type application/json", e);
                    return new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR);
                }
