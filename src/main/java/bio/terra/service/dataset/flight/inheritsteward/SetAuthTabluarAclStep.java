...
public record SetAuthTabluarAclStep(
  ...
  /**
   * Either set or remove the custodian email from the READER ACL of the snapshot's bigquery database.
   * <p/>
   * If inheritSteward is true, we use the dataset custodian's proxy email group to grant access
  ...
  private void setAuth(FlightContext context, boolean inheritSteward) throws InterruptedException {
    ...
    for (var snapshotId : Objects.requireNonNullElse(snapshotIds, List.<UUID>of())) {
