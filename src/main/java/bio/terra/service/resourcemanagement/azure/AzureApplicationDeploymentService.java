...
public class AzureApplicationDeploymentService {
    private static final Logger logger = LoggerFactory.getLogger(AzureApplicationDeploymentService.class);
     ...
     * @param billingProfile The billing profile model to use to connect to and read the managed metadata for
    ...
    public GenericResource retrieveApplicationDeployment(BillingProfileModel billingProfile) {
        AzureResourceManager client = resourceConfiguration.getClient(
            UUID.fromString(billingProfile.getSubscriptionId()));

        logger.info("Looking up application");
        String applicationResourceId = MetadataDataAccessUtils.getApplicationDeploymentId(billingProfile);
        return client.genericResources()
            .getById(applicationResourceId);
    }

    /**
     * Return the input parameters for the application deployment associated with the specified
    ...
    public Map<String, String> inputParameters(GenericResource applicationDeployment) {
        ...
        return StreamSupport.stream(
            Spliterators.spliteratorUnknownSize(properties.get(PARAMETERS_KEY).fields(), Spliterator.ORDERED), false)
            .collect(Collectors.toMap(
                Map.Entry::getKey,
                e -> e.getValue().get(PARAMETER_VALUE_KEY).asText()
            ));
    }

    /**
     * Register a new azure application deployment. This process is not transactional or done in a stairway flight,
     * so it is possible we will allocate projects and before they are recorded in our database, we
     * will fail and they will be orphaned.
     *
     ...
     */
    private AzureApplicationDeploymentResource newApplicationDeployment(BillingProfileModel billingProfile) {
        ...
        var managedResourceGroupName = managedResourceGroupPathParts[managedResourceGroupPathParts.length - 1];
        ...
        var resource = new AzureApplicationDeploymentResource()
            ...
            .storageAccountSkuType(AzureStorageAccountSkuType.fromAzureName(parameters.get(STORAGE_TYPE_KEY)));
