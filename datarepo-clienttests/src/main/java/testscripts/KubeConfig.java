...
public class KubeConfig extends runner.TestScript {
    ...
    // 2.5 files per minute, so two instances should do 5 files per minute. To run 5 minutes we should run
    // 25 files. (There are 25 files in the directory, so if we need more we should do a reuse scheme like the
    // fileLoadTest)
