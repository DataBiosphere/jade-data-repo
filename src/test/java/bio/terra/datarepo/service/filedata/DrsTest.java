...
import bio.terra.datarepo.integration.BigQueryFixtures;
import bio.terra.datarepo.integration.DataRepoClient;
...
public class DrsTest extends UsersBase {
  ...
  public void drsScaleTest() throws Exception {
    ...
    ConfigModel concurrentConfig = configurationService.getConfig(ConfigEnum.DRS_LOOKUP_MAX.name());

    concurrentConfig.setParameter(new ConfigParameterModel().value(failureMaxValue));
    ConfigGroupModel failureConfigGroupModel =
        new ConfigGroupModel().label("DRSTest").addGroupItem(concurrentConfig);

    List<ConfigModel> failureConfigList =
