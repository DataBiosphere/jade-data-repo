...
@Tag(Unit.TAG)
class SetServiceAccountConsumerUserStepTest {
    ...
    for (var projectId : projectIds) {
      if (grantPolicy) {
        verify(resourceService)
            .grantPoliciesServiceUsageConsumer(projectId, List.of(custodianEmail));
      } else {
        verify(resourceService)
            .revokePoliciesServiceUsageConsumer(projectId, List.of(custodianEmail));
      }
    }
  }
  ...
  void doStep(boolean inheritSteward) throws Exception {
    SetServiceAccountConsumerUserStep step =
        new SetServiceAccountConsumerUserStep(resourceService, custodianEmail, inheritSteward);
