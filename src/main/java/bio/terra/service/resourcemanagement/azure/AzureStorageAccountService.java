...
public class AzureStorageAccountService {
    ...
    /**
     * Fetch metadata for an existing storage_account_resource
     ...
     * @throws CorruptMetadataException if the storage_account_resource metadata row exists but the cloud
     * resource does not
     */
    @Transactional(propagation = Propagation.REQUIRED, isolation = Isolation.SERIALIZABLE, readOnly = true)
    public AzureStorageAccountResource getStorageAccountResourceById(UUID storageAccountResourceId,
                                                                     boolean checkCloudResourceExists) {
        ...
        if (checkCloudResourceExists) {
            ...
            StorageAccount storageAccount = getCloudStorageAccount(billingProfile, storageAccountResource);
    ...
    /**
     * Fetch/create a storage account cloud resource and the associated metadata in the storage_account_resource table.
     * <p>
     * On entry to this method, there are 9 states along 3 main dimensions:
     * Azure storage account - exists or not
     ...
     * In addition, there is one case where it matters if we are reusing storage accounts or not.
     * <p>
     * Itemizing the 9 cases:
     * CASE 1: storage account exists, record exists, record is unlocked
     * The predominant case. We return the storage account resource
     * <p>
     * CASE 2: storage account exists, record exists, locked by another flight
     * We have to wait until the other flight finishes creating the storage account. Throw StorageLockFailureException.
     * We expect the calling Step to retry on that exception.
     * <p>
     * CASE 3: storage account exists, record exists, locked by us
     * This flight created the storage account, but failed before we could unlock it. So, we unlock and
     * return the storage account resource.
     * <p>
     * CASE 4: storage account exists, no record exists, we are not reusing storage account
     ...
     * <p>
     * CASE 5: no storage account exists, record exists, not locked
     * This should not happen. Throw CorruptMetadataException
     * <p>
     * CASE 6: no storage account exists, record exists, locked by another flight
     * We have to wait until the other flight finishes creating the storage account. Throw
     * StorageAccountLockFailureException.
     * We expect the calling Step to retry on that exception.
     * <p>
     * CASE 7: no storage account exists, record exists, locked by this flight
     * We must have failed after creating and locking the record, but before creating the storage account.
     * Proceed with the finish-trying-to-create-storage-account algorithm
     * <p>
     * CASE 8: no storage account exists, no record exists
     * Proceed with try-to-create-storage-account algorithm
     * <p>
     * The algorithm to create a storage-account is like a miniature flight and we implement it as a set
     * of methods that chain to make the whole algorithm:
     * 1. createMetadataRecord: create and lock the metadata record; then
     * 2. createCloudStorageAccount: if the storage account does not exist, create it; then
     ...
     * @param applicationResource application deployment in which the storage account should be retrieved or created
    ...
    public AzureStorageAccountResource getOrCreateStorageAccount(String storageAccountName,
        ...
        AzureStorageAccountResource storageAccountResource =
            resourceDao.getStorageAccount(storageAccountName, applicationResource.getAzureApplicationDeploymentName());
        ...
        if (storageAccount != null) {
            if (storageAccountResource != null) {
                ...
                // CASE 4: This code as currently implemented is unreachable since an empty resource make it impossible
                // to have a cloud storage account value
                throw new CorruptMetadataException(
                    "Storage account already exists, metadata out of sync with cloud state: " + storageAccountName);
            }
        } else {
            ...
            if (storageAccountResource != null) {
                ...
                // CASE 7: this flight has the metadata locked, but didn't finish creating the storage account
                return createCloudStorageAccount(profileModel, storageAccountResource, flightId);
            } else {
                // CASE 8: no storage account and no record
                return createMetadataRecord(profileModel, storageAccountName, applicationResource, region, flightId);
            }
        }
    }
    ...
    private AzureStorageAccountResource createMetadataRecord(BillingProfileModel profileModel,
        ...
        AzureRegion regionToUse = Optional.ofNullable(region)
            .orElse(applicationResource.getDefaultRegion());

        AzureStorageAccountResource storageAccountResource =
            resourceDao.createAndLockStorageAccount(storageAccountName, applicationResource, regionToUse, flightId);
        if (storageAccountResource == null) {
    ...
    private AzureStorageAccountResource createCloudStorageAccount(BillingProfileModel profileModel,
                                                                  AzureStorageAccountResource storageAccountResource,
                                                                  String flightId) {
    ...
    private AzureStorageAccountResource createFinish(StorageAccount storageAccount,
                                              String flightId,
                                              AzureStorageAccountResource storageAccountResource) {
    ...
    /**
     * Create a new storage account cloud resource.
     * Note this method does not create any associated metadata in the storage_account_resource table.
     *
     ...
     */
    private StorageAccount newCloudStorageAccount(BillingProfileModel profileModel,
                                                  AzureStorageAccountResource storageAccountResource) {
        logger.info("Creating storage account '{}' in application deployment '{}'", storageAccountResource.getName(),
            storageAccountResource.getApplicationResource().getAzureApplicationDeploymentName());

        return resourceConfiguration.getClient(UUID.fromString(profileModel.getSubscriptionId()))
            .storageAccounts().define(storageAccountResource.getName())
            .withRegion(storageAccountResource.getRegion().getValue())
            .withExistingResourceGroup(storageAccountResource.getApplicationResource().getAzureResourceGroupName())
            .withSku(storageAccountResource.getApplicationResource().getStorageAccountSkuType().getValue())
            .withHnsEnabled(true)
            .create();
    }

    /**
     * Fetch an existing storage account cloud resource.
     * Note this method does not check any associated metadata in the storage_account_resource table.
     *
     * @param profileModel the TDR billing profile associated with this storage account
     * @param storageAccountResource storage account resource to look up storage account. If it is null, return null
     * @return a reference to the storage account as an Azure storage account object, null if not found
     */
    StorageAccount getCloudStorageAccount(BillingProfileModel profileModel,
                                          AzureStorageAccountResource storageAccountResource) {
        if (storageAccountResource == null) {
            return null;
        }
