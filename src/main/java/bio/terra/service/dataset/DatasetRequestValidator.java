...
public class DatasetRequestValidator implements Validator {
  ...
  private void validateTable(TableModel table, Errors errors, SchemaValidationContext context) {
    ...
    primaryKeyList.stream().forEach(pk -> {
      if (!columnNames.contains(pk)) {
        errors.rejectValue(
            "schema", "InvalidPrimaryKey", String.format("Primary key '%s' is not a valid column in table '%s'. Valid columns include: %s", pk, tableName, String.join(",", columnNames)));
      }
    });
