...
public class AzureDataLocationSelector {
    ...
    public String storageAccountNameForDataset(String prefix, String datasetName, BillingProfileModel billingProfile) {
    ...
    /**
     * Generate an n-character unique string in a way that mimics what azure's ARM template function uses.
    ...
    public static String armUniqueString(String seed, int size) {
        try {
            ...
            if (size <= 0) {
                throw new IllegalArgumentException("The size of the requested unique string must be greater than 0");
            }
