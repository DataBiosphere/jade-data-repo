...
from data_repo_client import Configuration, ApiClient, ProfilesApi, DatasetsApi, \
    SnapshotsApi, \
    JobsApi, SnapshotAccessRequestApi, ApiException
import argparse
import subprocess
import json
import os
...
class Clients:
    def __init__(self, host):
        ...
        config.host = host
        token_output = subprocess.run(['gcloud', 'auth', 'print-access-token'],
                                      capture_output=True)
        ...
        with open(os.path.join("files", dataset_to_upload["schema"],
            ...
            if len(recordsArray) > 0:
                ...
                wait_for_job(clients,
                             clients.datasets_api.ingest_dataset(dataset_id,
...
def create_billing_profile(clients, add_jade_stewards, cloud_platform,
    ...
    with open(
        os.path.join("profiles", billing_profile_file_name)) as billing_profile_json:
        ...
        if azure_managed_app_name:
            billing_profile_request[
                'applicationDeploymentName'] = azure_managed_app_name
            print(
...
def dataset_ingest_json(clients, dataset_id, dataset_to_upload):
    for table in dataset_to_upload['tables']:
        with open(os.path.join("files", dataset_to_upload["schema"],
            ...
            print(f"Ingesting data into {dataset_to_upload['name']}/{table}")
            wait_for_job(clients, clients.datasets_api.ingest_dataset(dataset_id,
                                                                      ingest=ingest_request))
...
def create_dataset(clients, dataset_to_upload, profile_id):
    ...
    with open(os.path.join("files", dataset_to_upload["schema"],
        ...
        dataset = wait_for_job(clients, clients.datasets_api.create_dataset(
    ...
    for snapshot_to_upload in snapshots:
        ...
        for i in range(count):
            ...
            snapshot = wait_for_job(clients,
                                    clients.snapshots_api.create_snapshot(
...
def create_snapshot_from_access_request(clients, snapshot_access_request_id):
    snapshot_request_model = get_file_to_upload(
        "snapshot-request-model-by-request-id.json")
    snapshot_request_model['requestIdSpec'][
        'snapshotRequestId'] = snapshot_access_request_id
    snapshot = wait_for_job(clients, clients.snapshots_api.create_snapshot(
        snapshot=snapshot_request_model))
    return snapshot['id']
...
def delete_dataset_if_exists(name, clients):
    ...
    if len(filtered_datasets) > 0:
        ...
        wait_for_job(clients,
                     clients.datasets_api.delete_dataset(filtered_datasets[0].id))
        print(f"Deleted dataset {filtered_datasets[0].id}")


def add_snapshot_builder_settings(clients, snapshot_id, directory,
                                  snapshot_builder_settings_file):
    with open(os.path.join("files", directory,
                           snapshot_builder_settings_file)) as snapshot_builder_settings_json:
        clients.snapshots_api.update_snapshot_snapshot_builder_settings(snapshot_id,
                                                                        json.load(
                                                                            snapshot_builder_settings_json))
...
def main():
    ...
    parser.add_argument('--datasets', required=True,
                        help="A file pointer to the datarepo datasets to create. This is required flag. Available Options: " + ", ".join(
                            os.listdir('./suites/')))
    ...
    parser.add_argument('--billing_profile_file_name',
                        help='A pointer to a file containing the billing profile to create or reuse. Provide either this or an existing profile ID.  Available Options: ' + ', '.join(
                            os.listdir('./profiles/')))
    ...
    outputs = []
    for dataset_to_upload in get_file_to_upload(args.datasets):
        snapshot_ids = []
        ...
        # print("Creating new snapshots from snapshot access requests")
        # for snapshot_access_request_id in snapshot_access_request_ids:
        #     snapshot_id = create_snapshot_from_access_request(clients,
        #                                                       snapshot_access_request_id)
        #     print(
        #         f"Created snapshot {snapshot_id} from snapshot access request {snapshot_access_request_id}")
