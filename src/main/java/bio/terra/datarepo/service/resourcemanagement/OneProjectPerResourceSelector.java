...
public class OneProjectPerResourceSelector implements DataLocationSelector {
  ...
  /**
   * *Borrowed from terra-resource-buffer* The size of project when generating random characters.
   * Choose size as 8 based on AoU's historical experience, increase if 8 is not enough for a pool's
   * naming.
   */
  static final int RANDOM_ID_SIZE = 8;

  @Autowired
  public OneProjectPerResourceSelector(
      GoogleResourceConfiguration resourceConfiguration, DatasetBucketDao datasetBucketDao) {
  ...
  private String getNewProjectId() throws GoogleResourceNamingException {
    ...
    // The project id below is an application level prefix or, if that is empty, the name of the
    // core project
    String projectId = resourceConfiguration.getDataProjectPrefixToUse() + projectDatasetSuffix;

    // Since the prefix can be set adhoc, let's check that the project name matches Google's
    // required pattern
