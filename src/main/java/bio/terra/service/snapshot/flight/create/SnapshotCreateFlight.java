...
public class SnapshotCreateFlight extends Flight {
  ...
  public SnapshotCreateFlight(FlightMap inputParameters, Object applicationContext) {
    ...
    Dataset firstDataset = sourceDatasets.get(0);
    // This is safe because we know all source datasets have the same platform and firestore region.
    var platform =
        CloudPlatformWrapper.of(firstDataset.getDatasetSummary().getStorageCloudPlatform());
    ...
    for (Dataset sourceDataset : sourceDatasets) {
      addStep(new LockDatasetStep(datasetDao, sourceDataset.getId(), false));
    }
    ...
    var snapshotContents = snapshotReq.getContents();
    for (int i = 0; i < snapshotContents.size(); i++) {
      addContentSteps(snapshotDao, snapshotService, bigQueryPdao, datasetService, snapshotReq, i);
    }
    ...
    addStep(
        new CreateSnapshotFireStoreDataStep(
            bigQueryPdao, snapshotService, dependencyDao, snapshotReq, fileDao, performanceLogger));
    ...
    if (platform.isGcp()) {
      for (int i = 0; i < snapshotContents.size(); i++) {
        addStep(
            new SnapshotAuthzFileAclStep(dependencyDao, snapshotService, gcsPdao, configService, i),
            pdaoAclRetryRule);
      }
    }
    ...
    // Unlock datasets
    for (Dataset sourceDataset : sourceDatasets) {
      addStep(new UnlockDatasetStep(datasetDao, sourceDataset.getId(), false));
    }
  }

  private void addContentSteps(
      ...
      SnapshotRequestModel snapshotReq,
      int sourceIndex) {
    var content = snapshotReq.getContents().get(sourceIndex);
    switch (content.getMode()) {
      case BYASSET:
        addStep(new CreateSnapshotValidateAssetStep(snapshotService, snapshotReq, sourceIndex));
        addStep(
            new CreateSnapshotPrimaryDataAssetStep(
                bigQueryPdao, snapshotDao, snapshotService, snapshotReq, sourceIndex));
        break;
      case BYFULLVIEW:
        addStep(
        ...
        break;
      case BYQUERY:
        addStep(new CreateSnapshotValidateQueryStep(datasetService, snapshotReq, sourceIndex));
        addStep(
