...
public final class PathUtils {
  ...
  public static List<Pair<Long, List<Long>>> computePaths(
    ...
    List<Pair<Long, List<Long>>> nodePaths =
        allNodes.stream()
            .map(node -> Pair.<Long, List<Long>>of(node, new ArrayList<Long>()))
            .toList();
    ...
    for (int ctr = 0; ctr < maxPathLengthHint; ctr++) {
      for (var nodePath : nodePaths) {
        ...
        childParentRelationships.stream()
            .filter(
                relationship ->
                    (long) relationship.getKey()
                        == (path.isEmpty() ? nodePath.getKey() : path.get(path.size() - 1)))
   ...
   */
  public static Map<Long, Long> countChildren(
      Collection<Pair<Long, Long>> childParentRelationships) {
