...
public class AzureApplicationDeploymentServiceTest {
    ...
    public void testGetOrRegisterApplicationDeployment() {
        ...
        when(genericResource.properties()).thenReturn(Map.of(
            MANAGED_RESOURCE_GROUP_ID_KEY, "mgd-grp-1",
            PARAMETERS_KEY, Map.of(
        ...
        when(genericResources.getById("/subscriptions/" + billingProfileModel.getSubscriptionId() + "/resourceGroups" +
            "/" + billingProfileModel.getResourceGroupName() + "/providers/Microsoft.Solutions/applications/" +
            billingProfileModel.getApplicationDeploymentName())).thenReturn(genericResource);
        when(client.genericResources()).thenReturn(genericResources);
        when(resourceDao.retrieveApplicationDeploymentByName(billingProfileModel.getApplicationDeploymentName()))
        ...
        assertThat("Application name matches", appResource.getAzureApplicationDeploymentName(),
            equalTo(billingProfileModel.getApplicationDeploymentName()));
        assertThat("Managed resource group matches", appResource.getAzureResourceGroupName(),
            equalTo("mgd-grp-1"));
        assertThat("Default region matches", appResource.getDefaultRegion(),
            equalTo(AzureRegion.AUSTRALIA));
        assertThat("Prefix matches", appResource.getStorageAccountPrefix(),
            equalTo("tdr"));
        assertThat("Storage account type matches", appResource.getStorageAccountSkuType(),
