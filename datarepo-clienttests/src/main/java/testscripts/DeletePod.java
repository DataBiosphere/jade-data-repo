...
public class DeletePod extends runner.TestScript {
  ...
  public void userJourney(ApiClient apiClient) throws Exception {
    ...
    if (bulkLoadArrayJobResponse.getJobStatus().equals(JobModel.JobStatusEnum.RUNNING)) {
      logger.debug("Delete a random pod");
      KubernetesClientUtils.deleteRandomPod();

      bulkLoadArrayJobResponse =
          DataRepoUtils.pollForRunningJob(repositoryApi, bulkLoadArrayJobResponse, 45);

      if (bulkLoadArrayJobResponse.getJobStatus().equals(JobModel.JobStatusEnum.RUNNING)) {
        logger.debug("Delete another random pod");
        KubernetesClientUtils.deleteRandomPod();
      }
    }
    /*try {
        ...
        logger.debug(
            "Catching expected exception while pod deleteing, Job Status: {}",
