...
public class AzureResourceDao {
    ...
    private static final String sqlApplicationRetrieve = "SELECT id, azure_application_deployment_id, " +
    ...
    private static final String sqlApplicationRetrieveById = sqlApplicationRetrieve +
        " WHERE marked_for_delete = false AND id = :id";
    private static final String sqlApplicationRetrieveByDepName = sqlApplicationRetrieve +
        " WHERE marked_for_delete = false AND azure_application_deployment_name = :azure_application_deployment_name";
    private static final String sqlApplicationRetrieveByIdForDelete = sqlApplicationRetrieve +
        " WHERE marked_for_delete = true AND id = :id";
    private static final String sqlApplicationRetrieveByBillingProfileId = sqlApplicationRetrieve +
    ...
    public UUID createApplicationDeployment(AzureApplicationDeploymentResource application) {
        String sql = "INSERT INTO application_deployment_resource " +
        ...
        MapSqlParameterSource params = new MapSqlParameterSource()
            .addValue("azure_application_deployment_id", application.getAzureApplicationDeploymentId())
            .addValue("azure_application_deployment_name", application.getAzureApplicationDeploymentName())
            ...
            .addValue("storage_account_sku_type", application.getStorageAccountSkuType().name().toString())
    ...
    private AzureApplicationDeploymentResource retrieveApplicationDeploymentBy(String sql,
                                                                               MapSqlParameterSource params) {
        List<AzureApplicationDeploymentResource> applicationList = retrieveApplicationDeploymentListBy(sql, params);
        if (applicationList.size() == 0) {
            throw new AzureResourceNotFoundException("Application deployment not found");
        }
        if (applicationList.size() > 1) {
            throw new CorruptMetadataException("Found more than one result for deployed application resource: " +
                applicationList.get(0).getAzureApplicationDeploymentName());
        }
        return applicationList.get(0);
    }

    private List<AzureApplicationDeploymentResource> retrieveApplicationDeploymentListBy(String sql,
                                                                                       MapSqlParameterSource params) {
        return jdbcTemplate.query(sql, params, (rs, rowNum) -> new AzureApplicationDeploymentResource()
            ...
            .storageAccountSkuType(AzureStorageAccountSkuType.valueOf(
    ...
    /**
     * Insert a new row into the storage_account_resource metadata table and give the provided flight the lock by
     * setting the flightid column. If there already exists a row with this storage account name, return null instead
    ...
    public AzureStorageAccountResource createAndLockStorageAccount(
        ...
        String sql = "INSERT INTO storage_account_resource (application_resource_id, name, datacontainer, " +
    ...
    /**
     * Unlock an existing storage_account_resource metadata row, by setting flightid = NULL.
    ...
    public void unlockStorageAccount(String storageAccountName, String flightId) {
        String sql = "UPDATE storage_account_resource SET flightid = NULL " +
            "WHERE name = :name AND flightid = :flightid";
        MapSqlParameterSource params = new MapSqlParameterSource()
            .addValue("name", storageAccountName)
            .addValue("flightid", flightId);
        int numRowsUpdated = jdbcTemplate.update(sql, params);
        logger.info("Storage account {} was {}", storageAccountName, (numRowsUpdated > 0 ? "unlocked" : "not locked"));
    }

    /**
     * Fetch an existing storage_account_resource metadata row using the name amd application deployment name.
     * This method expects that there is exactly one row matching the provided name and application name.
     *
     ...
     * @return a reference to the storage account as a POJO {@link AzureStorageAccountResource} or null if not found
     * @throws AzureResourceException  if the storage account matches, but is in the wrong application deployment
     * @throws CorruptMetadataException if multiple storage accounts have the same name
     */
    @Transactional(propagation = Propagation.REQUIRED, readOnly = true)
    public AzureStorageAccountResource getStorageAccount(String storageAccountName, String applicationName) {
        ...
        if (!Objects.equal(foundApplicationName, applicationName)) {
            // there is a storage account with this name in our metadata, but it's for a different app deployment
            throw new AzureResourceException(
                String.format("A storage account with this name already exists for a different application: %s, %s",
    ...
    /**
     * Fetch an existing storage_account_resource metadata row using the id.
    ...
    public AzureStorageAccountResource retrieveStorageAccountById(UUID storageAccountId) {
        ...
        if (storageAccountResource == null) {
            throw new AzureResourceNotFoundException("Storage account not found for id:" + storageAccountId);
        }
        return storageAccountResource;
    }

    /**
     * Delete the storage_account_resource metadata row associated with the storage account, provided the row is either
    ...
    public boolean deleteStorageAccountMetadata(String storageAccountName, String flightId) {
        String sql = "DELETE FROM storage_account_resource " +
    ...
    private AzureStorageAccountResource retrieveStorageAccountBy(String sql, MapSqlParameterSource params) {
        List<AzureStorageAccountResource> storageAccountResources =
            jdbcTemplate.query(sql, params, (rs, rowNum) -> {
                // Make deployed application resource and a storage account resource from the query result
                AzureApplicationDeploymentResource applicationResource = new AzureApplicationDeploymentResource()
                ...
                AzureRegion region = Optional.ofNullable(rs.getString("region"))
        ...
        if (storageAccountResources.size() > 1) {
            throw new CorruptMetadataException("Found more than one result for storage account resource: " +
