...
def add_snapshot_builder_settings(
    clients, dataset_id, directory, snapshot_builder_settings_file
):
    with open(
        os.path.join("files", directory, snapshot_builder_settings_file)
    ) as dataset_schema_json:
        clients.datasets_api.update_dataset_snapshot_builder_settings(
            dataset_id, json.load(dataset_schema_json)
        )
